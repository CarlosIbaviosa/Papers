...
# Group "exposure duration" and "exemplar variation" paragraphs together...
As previously mentioned, one of the main factors affecting our ability to recognise faces is how familiar we are with the face in question. Previous studies have suggested that both our accuracy and confidence increase when viewing a face for a longer, compared to shorter duration (Memon, Hope, & Bull, 2003)

Several studies have suggested that object recognition at least partially depends on viewing time - intuitively, we are better at recognising objects when given more time to assess the details of an image (faces and fingerprints). However, recently researchers have also suggested that recognition depends on how many different exemplars we view of a particular object - the more exemplars we view, the better (Murphy et al., 2015). Recent research has suggested that exposure to varying instances operates independently from viewing time (references). However, these studies nevertheless gave participants the freedom to assess the images as they pleased - allowing participants to still process the details of several images. In other words, we know for certain that processing varying exemplars of images can assist in recognition; however, given the surprisingly high error rate when identifying stangers' faces even under optimal conditions, and the benefit of assessing the details of a face for a logner duration, our ability to learn a strangers' face when presented only for a few milliseconds is unknown. Our study aims to take this a step further, and asks whether greater exposure to varying instances still facilitates recognition when we have no time to process a single image.

Additionally, the benefit of careful analysis within the laboratory suggests that this is not just an untested forensic procedure - one experiment by Thompson and Tangen \colorbox{yellow}{(Experiment 3; 2014)} found that when examining highly similar, non-matching fingerprints (??) for two seconds compared to 60 seconds, both experts and novices performed better when given more time to examine the fingerprints. This suggests that a major factor influencing identification accuracy may be the opportunity to carefully analyse different fingerprints.
