...
Previous research has suggested several factors that influence performance in visual recognition tasks. With regards to face matching, one of the most prominent factors is that of inversion. Despite generally being considered experts in processing upright faces \colorbox{yellow}{(references)}, we typically perform much worse when identifying or recognising inverted faces (see Rossion, 2008 for a review). Indeed, when we view upright faces, we typically view the entire image more holistically (Farah, Wilson, Drain, & Tanaka, 1998; Ingvalson & Wenger, 2005); but when viewing inverted faces, we process each facial feature separately (Tanaka & Simonyi, 2016; see also Rossion, 2008 for a review). While some authors suggest that this face inversion effect may be due to an innate attentiveness to upright faces (and similar-looking objects) \colorbox{yellow}{(references)}, others suggest that this effect is due to our relative inexperience in viewing inverted faces (references), as holistic processing seems to depend on familiarity with a stimulus class (e.g., Campbell & Tanaka, 2018; Tanaka & Simonyi, 2016; Wong et al., 2009; but see McKone & Robbins, 2007 for a critique). Although no studies have examined the effect of face inversion in an RSVP stream of similar faces (but see Haberman et al., 2009; Haberman, Lee, & Whitney, 2015; and Leib, Puri, Fischer, Bentin, Whitney, & Robertson, 2012, in relation to ensemble coding generally), the face inversion effect has been widely studied in the context of visual memory and other visual processing tasks (see Tanaka & Simonyi, 2016, and Valentine, 1988 for reviews) and so it seems plausible that it may influence identification performance in our study.
...
# Experiment 1
## Methods
The preregistration plan [add link] for this study is available on the Open Science Framework (OSF), and includes my predictions and hypotheses, methodology, power analysis, analysis plan, and links to all available materials, software, raw data files, and R markdown scripts.
