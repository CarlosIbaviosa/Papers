...
Our ability to correctly categorise an object or image seems to depend on how much experience we have in viewing similar kinds of objects in the first place. For example, the prototype theory of categorisation suggests that when categorising an object, we compare it to the typical representation of similar objects in our long-term memory and categorise it accordingly \colorbox{yellow}{(reference)}. Similarly, the exemplar theory of categorisation suggests that, when recognising an object, we compare it to our memories of specific objects within a particular category that we have accumulated in the past \colorbox{yellow}{(references)}, and search for similarities. Due to this reliance on similar prior experiences, it tends to be more difficult to categorise objects that we do not see very often, because we are not familiar with how these objects may vary under different contexts, or are unaware of the more stable, average characteristics among these objects that may facilitate categorisation \colorbox{yellow}{(reference)}. The influence of experience on visual identification and memory tasks has been studied with expert-novice comparisons in a variety of visual domains, such as chess board configurations (references), bird species (references), or car models (references) - however, a substantial body of literature has particularly focused on the role of familiarity in face recognition. Indeed, trying to identify a stranger’s face proves much more difficult than identifying a friend’s face or a celebrity’s face, because we do not have an average representation of how a stranger's face typically looks across contexts, and may mistake simple variations in lighting or hairstyle for complete changes in identity \colorbox{yellow}{(references)}.

Determining the average properties of similar images is often referred to as deriving the “ensemble” of a set of images \colorbox{yellow}{(references)}. Many studies focused on ensemble coding suggest that we automatically create ensemble representations as we view similar objects. In fact, many studies suggest that we do not even need to commit each individual image into our long-term memory for this to happen - we can derive an ensemble from a rapidly presented series of images, just by processing the gist of each image alone \colorbox{yellow}{(references)}. Indeed, many studies using the rapid serial visual presentation (RSVP) methodology, where a series of similar images are presented rapidly (i.e., several milliseconds each) at a fixed location, have shown that we can automatically compute the average representation of all of the images, despite having no time to commit any image into visual memory (references). This finding has been replicated for when participants focus on simple stimuli (reference), complex stimuli (reference), and even when participants are completing another task with the RSVP stream in the background (reference). However, while ensemble coding is clearly very robust to task demands, and it seems intuitively linked to how we become familiar with new images, no studies seem to have established whether presenting unfamiliar images in an RSVP stream can help to identify new images of the same category.  The current study, therefore, asks whether viewing the gist of several images can improve novices' ability to identify unfamiliar objects - in particular, strangers’ faces and fingerprints - compared to carefully assessing the details of a single image; and, if it does, how might these benefits interact with other variables that influence recognition in these contexts.
