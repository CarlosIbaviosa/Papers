...
Our ability to correctly categorise an object or image seems to depend on how much experience we have in viewing similar kinds of objects in the first place. For example, the prototype theory of categorisation suggests that when categorising an object, we compare it to the typical representation of similar objects in our long-term memory and categorise it accordingly \colorbox{yellow}{(reference)}. Similarly, the exemplar theory of categorisation suggests that, when recognising an object, we compare it to our memories of specific objects within a particular category that we have accumulated in the past \colorbox{yellow}{(references)}, and search for similarities. Due to this reliance on similar prior experiences, it tends to be more difficult to categorise objects that we do not see very often, because we are not familiar with how these objects may vary under different contexts, or are unaware of the more stable, average characteristics among these objects that may facilitate categorisation \colorbox{yellow}{(reference)}. On the particular level, for example, a substantial body of literature has focused on the role of familiarity in individual face recognition. Indeed, trying to identify a stranger’s face proves much more difficult than identifying a friend’s face or a celebrity’s face, because we do not know what a stranger's face typically looks like on average and how it varies across contexts, and may mistake simple variations in lighting or hairstyle for complete changes in identity \colorbox{yellow}{(references)}. This is not the case for familiar faces, where we can draw on our prior memories of that face across contexts, and easily recognise them in a new environment (references). However, the role of experience in aiding recognition can also apply to broader, category-level judgements. For example, many studies have demonstrated that while experts excel in categorising or identifying objects within their particular visual domain, this expertise does not necessarily translate to similar objects outside their domain of expertise. Modern car experts cannot as easily identify antique cars (references), chess masters can remember legal chess configurations but not random configurations (references), and while the average person can typically process upright faces very efficiently, we struggle to do the same with inverted faces (references).

If understanding the regularities between similar visual stimuli assists in recognition and categorisation tasks, both on an individual and category level, how exactly do we assess these regularities? One related process seems to be "ensemble coding", which allows us to glean the average properties of a range of similar stimuli and make sense of redundancies in our environment \colorbox{yellow}{(references)}. Indeed, many studies suggest that this process is so automatic that we do not even need to commit each individual image into our long-term memory for this to happen - we can derive an ensemble from a set of images, just by processing the gist of each image alone \colorbox{yellow}{(references)}. For example, many studies using the rapid serial visual presentation (RSVP) methodology, where a series of similar images are presented rapidly (i.e., several milliseconds each) at a fixed location, have shown that we can automatically compute the average representation of all of the images, despite having no time to commit any image into visual memory (references). This finding has been replicated for when participants focus on simple stimuli (reference), complex stimuli (reference), and even when the RSVP stream is not the main focus of the experiment (reference). However, while ensemble coding is clearly very robust to task demands, and it seems intuitively linked to how we become familiar with new images, no studies seem to have established whether presenting unfamiliar images in an RSVP stream can help to identify new images of the same category. The current study, therefore, asks whether rapidly viewing the gist of several images can improve novices' ability to identify unfamiliar objects compared to carefully assessing the details of a single image. In this case, we will use strangers' faces and fingerprints as visual stimuli. We will also explore the possible constraints to this methodology when considering other variables that may influence recognition in these contexts.
