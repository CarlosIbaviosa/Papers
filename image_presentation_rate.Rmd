...
Our ability to correctly categorise an object or image seems to depend on how much experience we have in viewing similar kinds of objects in the first place. For example, the prototype theory of categorisation suggests that when categorising an object, we compare it to the typical representation of similar objects in our long-term memory and categorise it accordingly \colorbox{yellow}{(reference)}. Similarly, the exemplar theory of categorisation suggests that, when recognising an object, we compare it to our memories of specific objects within a particular category that we have accumulated in the past \colorbox{yellow}{(references)}, and search for similarities. Due to this reliance on similar prior experiences, it tends to be more difficult to categorise objects that we do not see very often, because we are not familiar with how these objects may vary under different contexts, or are unaware of the more stable, average characteristics among these objects that may facilitate categorisation \colorbox{yellow}{(reference; also examples of experts (i.e., chess, birds, fingerprints) being able to rapidly recognise/categorise relevant objects based on experience compared to novices)}. When asked to recall legal \colorbox{yellow}{(?)} chess board configurations, for example, chess novices perform worse than experts, because they do not have the same cumulative knowledge of chess configurations as experts do (references). Similar findings have been reported with a range of different visual stimuli, such as birds (references) or cars (references), and a substantial body of literature has particularly focused on the role of familiarity in face recognition. Indeed, trying to identify a stranger’s face proves much more difficult than identifying a friend’s face or a celebrity’s face, because we do not have an average representation of how a stranger's face typically looks across contexts, and may mistake simple variations in lighting or hairstyle for complete changes in identity \colorbox{yellow}{(references)}.

Determining the average properties of similar images in memory is often referred to as deriving the “ensemble” of a set of images \colorbox{yellow}{(references)}. Many studies focused on ensemble coding suggest that we automatically create ensemble representations as we view similar objects - even when the stimuli are rather complex, such as faces \colorbox{yellow}{(references - and maybe provide other examples of robustness...)}. In fact, many studies suggest that we do not even need to commit each individual image into our long-term memory for this to happen - we can derive an ensemble from a rapidly presented series of images, just by processing the gist of each image alone \colorbox{yellow}{(references)}. Indeed, many studies using the rapid serial visual presentation (RSVP) methodology, where a series of similar images are presented rapidly (i.e., several milliseconds each) at a fixed location, have shown that we can automatically compute the average representation of all of the images, despite having no time to commit any image into visual memory (references). This finding has been replicated for when participants focus on simple stimuli (reference), complex stimuli (reference), and even when participants are completing another task with the RSVP stream in the background (reference)! However, while many studies have  suggested that recognising a complex, unfamiliar image, such as a stranger's face, may benefit from viewing several similar photos \colorbox{yellow}{(references)}, no research seems to have examined whether we can learn to identify an unfamiliar image purely based on a gist-based ensemble, using the RSVP methodology. The current study, therefore, asks whether novices can learn to better recognise unfamiliar objects - in particular, strangers’ faces and fingerprints - using this methodology, and how it may interact with other variables that influence object recognition.
...
However, it is not necessarily the case that we become more familiar with a face when viewing more photos of that person. Indeed, \colorbox{yellow}{(other references)} suggests no significant advantage in face recognition when participants viewed three or 16 photos of a stranger - despite viewing five times more photos in the second condition. Some authors have then suggested that exposure to a high degree of contextual variability, rather than the sheer number of photos themselves, may be more important when becoming familiar with a new face \colorbox{yellow}{[examples; references]. [maybe lead into hypothesis 1, linking this back to the main narrative????]}
 ...
 When identifying a familiar face, however, our cumulative memories of what the face looks like on average allows us to see through superficial changes in context, and we can easily assess whether the stable features of their face in a new context align with our representation of that face in long-term memory (Bruce & Young, 1986; Burton & Bruce, 1993).
