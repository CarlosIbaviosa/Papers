...
# Experiment 2
Experiment 2 employs a similar design to Experiment 1; however, to more closely resemble fingerprint identification procedure, where examiners carefully mark up a latent fingerprint before examining the suspect prints, participants were shown the target image of a crime scene print first, before viewing the RSVP stream or single comparison print. While this may change the nature of how beneficial the subsequent ensemble representation may be, previous research using the RSVP methodology suggests that when participants are primed to recognise a particular image among a subsequently presented image stream of random images, performance improves drastically, as they now know what to look for \colorbox{yellow}{(reference)}. Accordingly, similar to Experiment 1 we predict that performance will improve when viewing more rapid image streams. Additionally, instead of presenting fingerprints in an upright or inverted orientation as in Experiment 1, Experiment 2 manipulates whether participants viewed fingerprints belonging to the same finger (i.e., "Is this John's thumb?"), or to the same person more generally (i.e., "Does this fingerprint belong to John?"), as this manipulation allows us to simulate the kinds of "ten-print" materials that fingerprint examiners typically have at their disposal (reference). In doing so, we can examine whether the potential benefits of an RSVP stream are constrained by the specificity level of the identification. While evidence suggests that novices may perform similarly when discriminating prints from the same person and same finger (see Searston & Tangen, 2017c, Tangen et al., 2011, and Tangen et al., 2014), RSVP streams consisting of prints belonging to the same finger may contain less variation compared to prints from different fingers from the same person, and therefore may generate a more stable ensemble with which to compare the latent print, and boost recognition (see Whitney & Leib, 2018). We therefore predict that any benefits derived from the RSVP methodology may be more pronounced when viewing streams of the same finger.

## Method
In this experiment, participants viewed single images of a latent crime scene fingerprint before viewing a stream of fully-rolled fingerprint images. They then determined whether the fingerprints in the stream belonged to the same or different finger, or the same or different person more broadly, to the latent fingerprint (see Figure 5 and Figure 6). As in Experiment 1, presentation rate varied for each stream, and participants’ confidence and discriminability were the main performance measures of interest. This experiment was preregistered along with Experiment 1.
...
### Software
The software for Experiment 2 was identical to that in Experiment 1, and all relevant files are available under the same pre-registration link.
...
As in Experiment 1, we observed no significant difference in confidence across image rates, contrary to our prediction. Despite the relative advantages in each condition, it seems that the task demands in this experiment were too difficult for participants to feel confident - fingerprint matching tasks are quite challenging for novices (Searston & Tangen, 2017c; Tangen et al., 2011; Thompson et al., 2014), and would likely still be challenging when viewing an RSVP stream.

## General Discussion
- what are the 3-5 main findings? (plan)
  - RSVP works if actually presented like a normal identification task (for both familiar and unfamiliar objects)
  - RSVP/ensemble coding indeed works with complex, unfamiliar image categories... and probably also for images with differing specificity levels
  - comment on how this might add to the confidence/accuracy literature
  - Exp 2 was more of a working memory task... may be useful for demonstrating expertise, but not training expertise
  - comment on exp 2, and how Potter's finding is distinct from our finding (and why)
...
Our experiments assessed whether using variants of an RSVP methodology could increase performance in identification tasks with complex images, compared to viewing single images for a longer duration. When examining face recognition in Experiment 1 and presenting the test image after the image stream, we found that identification increased as image rate increased, with both upright and inverted faces; but we found the opposite pattern of results when presenting the test image before the image stream in Experiment 2, when participants viewed fingerprints. Given that the two experiments were fairly different, it is difficult to pinpoint the source of these differences - it is possible that fingerprint images are simply more difficult for novices to process than (even inverted) faces, or that there were influential differences between the stimulus sets, such as restricted image variability or the number of available images in Experiment 2. However, we believe that the major distinguishing factor between these experiments is the fact that we presented the test image after the RSVP streams in Experiment 1, but *before* the streams in Experiment 2. The literature on image recognition generally states that we can more easily identify images that we are familiar with, compared to those we are unfamiliar with - because our familiarity with an image can make it easier to compare subsequent images with our previous memories of different exemplars, or our average understanding of those exemplars more generally. While no research has applied an RSVP methodology before, our experiments are ultimately image recognition tasks - and so it makes sense that identification improved in Experiment 1, which more closely aligns with our understanding of familiarisation and identification, compared to Experiment 2. While we cannot be certain, it seems reasonable to assume that if Experiment 2 more closely followed the format of Experiment 1, we may see a similar increase in identification performance with image rate. Indeed, Experiment 1 suggested that the RSVP methodology can facilitate recognition with inverted faces - which the literature suggests are processed as unfamiliar, non-face objects, with detrimental effects on holistic processing and identification performance - and so it is still possible that this could hold true for fingerprints under more similar conditions.

One way to explain our discrepant results between experiments is through varying degrees of familiarity between the image categories. While we interpreted performance with inverted faces in Experiment 1 as evidence that the RSVP methodology would work with unfamiliar image categories, Experiment 2 suggests that this may not work with completely novel image categories. Although no study seems to have obtained reliable results comparing novice performance with fingerprints and inverted faces (see Searston & Tangen, 2017 - task vs. class), familiarity is a spectrum, rather than a dichotomy, and while we may be most familiar with upright faces, our daily exposure to faces and innate ability to process face-like objects may nevertheless make inverted faces seem more familiar than fingerprints. Given that the RSVP methodology seems to depend somewhat on holistic processing and gist perception (see Oliva, 2005), which in themselves seem linked to category familiarity (e.g., Campbell & Tanaka, 2018; Wong et al., 2009), it is possible that the potential benefits of an RSVP methodology for identification are relative to the degree of familiarity with the image category more broadly - if the category is too unfamiliar, then the RSVP methodology stops being beneficial. If this is the case, future research may wish to adjust the image rate to suit the relative unfamiliarity of the selected image category.

This difference in overall discriminability may be due to the fact that Experiment 1 had a higher degree of image variation than Experiment 2. In Experiment 1, all images were coloured and blurred and consisted of people in different contexts, including the subsequent test images; however, in Experiment 2 the stream images were somewhat controlled and artificial (i.e., fully-rolled prints, all on a white background) compared to the latent crime scene prints, which may vary in different ways to the prints used in the stream (e.g., contact surface or print pressure). That is, the streams in Experiment 1 were a closer match to the test images than in Experiment 2. Previous research in face recognition suggests that exposure to more variable images better facilitates recognition in a new context compared to less variable images (Menon, White, & Kemp, 2015; Ritchie & Burton, 2017), and so it is possible that the more controlled nature of the stream images in Experiment 2 may have hindered participants’ ability to recognise the test images compared to the more variable stream images in Experiment 1. However, Ritchie and Burton (2017) suggest that [viewing multiple similar images, even with (?)] reduced variability should nevertheless increase rather than decrease recognition compared to viewing single images. As such, while reduced variability may explain why participants did not benefit from the print streams in Experiment 2, it does not account for the significant decrease in discriminability observed with increasing presentation rates. Of course, it is possible that a combination of the aforementioned design factors may have produced the opposite trends observed across the two experiments.
...
While the current study sheds light on our ability to identify new instances of unfamiliar images, using images commonly used by forensic examiners, this methodology cannot be directly extrapolated into every forensic case. The number of images available to forensic examiners for any given identity and category may drastically limit how applicable this methodology can be to real cases - for example, fingerprint examiners typically would not have access to so many fully-rolled prints from the same finger for any given suspect (reference). However, modified versions of this methodology may still be useful for developing training procedures in a number of forensic disciplines. For example, in modifying Experiment 2, previous research suggests that images presented in streams of at least one second per image can be efficiently remembered for long periods (e.g., Potter & Levy, 1969; Standing, 1973); and Thompson and Tangen (2014, Experiment 3) suggested that viewing a fingerprint for two seconds only incurred a 6.8 percent decrease in accuracy for novices compared to viewing prints for one minute. It is possible, therefore, that if we repeated Experiment 2 with slower image rates this may optimally balance the advantages of both viewing the detail in a single image and being exposed to variability within images with novices. Future research may wish to either decrease the presentation rate, or allow participants themselves to control presentation rate and view each fingerprint for as long as they deem necessary for familiarisation. The latter manipulation would preserve individual differences in evidence accumulation styles (i.e., some people may prefer more image variation, while others may prefer more viewing time), providing a less intrusive method of investigating how presentation rate might predict identification performance.

Some evidence suggests that experts process images more holistically than novices (Busey & Vanderkolk, 2005; but see Vogelsang, Palmeri, & Busey, 2017 for a competing study); and if this is the case, it seems likely that presenting this RSVP methodology may be more useful when administered during later stages of training. Given that previous research suggests that the majority of learning among fingerprint novices occurs within the first three months of training (Searston & Tangen, 2017b), it is possible that an RSVP methodology for identification may be most beneficial after the initial learning phase. [inverted faces??]
...
- Applications: fingerprint RSVP methodology could still work if changed around - even though it’s a completely novel stimulus, participants still performed better than chance, so if we made it a little easier then it may still serve as an effective training tool for increasing familiarity/expertise in a set amount of time
