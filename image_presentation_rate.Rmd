...
Our ability to correctly categorise an object or image seems to depend on how much experience we have in viewing similar kinds of objects in the first place. For example, the prototype theory of categorisation suggests that when categorising an object, we compare it to the average representation of similar objects in our long-term memory and categorise it accordingly \colorbox{yellow}{(reference)}. That is, we may recognise that a samoyed is a dog, despite never having seen a samoyed before, because the ‘dog’ prototype represented in our memory tells us that dogs have four legs, a tail, and particular facial features that this adorable snowball also seems to have. Similarly, the exemplar theory of categorisation suggests that, when recognising an object, we compare it to our memories of specific objects within a particular category that we have accumulated in the past \colorbox{yellow}{(references)} - that is, we may recognise that a stranger’s dog is not just a dog, but a samoyed, because it looks similar to other samoyeds we have seen in the past.

Due to this reliance on similar prior experiences, it tends to be more difficult to categorise objects that we do not see very often. Categorising an object based on a single comparison image, for example, tends to be very difficult because we are not familiar with how this object may vary under different contexts, or are unaware of the more stable characteristics that may facilitate categorisation \colorbox{yellow}{(reference; also examples of experts (i.e., chess, birds, fingerprints) being able to rapidly recognise/categorise relevant objects based on experience compared to novices)}. Indeed, many studies have suggested that this occurs when classifying any object or image, such as \colorbox{yellow}{[give examples?]} - and in particular, many studies have focused on how this may occur in the context of face recognition. Indeed, trying to identify a stranger’s face proves much more difficult than identifying a friend’s face or a celebrity’s face, because we are unfamiliar with how the stranger’s face may look under different lighting conditions, or when their hair is styled differently, or when they are wearing sunglasses \colorbox{yellow}{(references)}. When identifying a familiar face, however, we already have an expansive repository of similar experiences with that face across different contexts, and we can automatically assess whether the stable features of their face in a new context align with our visual representation of that face in long-term memory (Bruce & Young, 1986; Burton & Bruce, 1993).

Determining the average properties of similar images in memory is often referred to as deriving the “ensemble” of a set of images \colorbox{yellow}{(references)}. Many studies focused on ensemble coding suggest that we automatically create ensemble representations as we view similar objects - even when the stimuli are rather complex, such as faces \colorbox{yellow}{(references - and maybe provide other examples of robustness...)}. In fact, many studies suggest that we do not even need to commit each individual image into our long-term memory for this to happen - we can derive an ensemble from a rapidly presented series of images (i.e., several hundred milliseconds each), just by processing the gist of each image alone \colorbox{yellow}{(references)}. While many studies have previously suggested that recognising a complex, unfamiliar image, such as a face, may benefit from being able to assess several photos of similar images \colorbox{yellow}{(references)}, no research seems to have examined whether we can learn to identify an unfamiliar image purely based on a gist-based ensemble, using the RSVP \colorbox{yellow}{[make sure you introduce this term]} methodology. The current study, therefore, asks whether novices can learn to better recognise unfamiliar objects - in particular, strangers’ faces and fingerprints - using this methodology, and how it may interact with other variables that influence object recognition.

## Factors affecting face recognition
As previously mentioned, one of the main factors affecting our ability to recognise faces is whether we are familiar with the person or not. While we can easily recognise a familiar face across different contexts - even when the image is blurred, etc. \colorbox{yellow}{(references)} - several studies have revealed an alarmingly high error rate when trying to identify an unfamiliar face, even under optimal conditions \colorbox{yellow}{(i.e., elaborate…; references)}. That is, when trying to identify a stranger based on a single comparison photo, some studies have reported an error rate ranging from \colorbox{yellow}{x to x}, even among professional face examiners \colorbox{yellow}{(references)}. This is likely due to having a richer ensemble representation with which to compare familiar faces, as some authors suggest that this ensemble may specifically consist of more stable, internal features when recognising a familiar face, such as the eyes, nose, or mouth, and that, absent of this ensemble, we depend more on external features such as hairstyle when attempting to identify an unfamiliar face \colorbox{yellow}{(references)} - and likewise are more affected by variations in external features between images of unfamiliar faces \colorbox{yellow}{(references)}.
However, it is not necessarily the case that we become more familiar with a face when viewing more photos of that person. Indeed, \colorbox{yellow}{(other references)} suggests no significant advantage in face recognition when participants viewed three or 16 photos of a stranger - despite viewing five times more photos in the second condition. Some authors have then suggested that exposure to a high degree of contextual variability, rather than the sheer number of photos themselves, may be more important when becoming familiar with a new face \colorbox{yellow}{[examples; references]. [maybe lead into hypothesis 1, linking this back to the main narrative????]}

Another factor affecting face recognition is the orientation of the image. Despite generally being considered experts in processing upright faces \colorbox{yellow}{(references)}, we typically perform much worse when identifying or recognising inverted faces (see Rossion, 2008 for a review). Indeed, when we view upright faces, we typically view the entire image more holistically (i.e., “I see a face”; Farah, Wilson, Drain, & Tanaka, 1998; Ingvalson & Wenger, 2005); but when viewing inverted faces, we process each facial feature separately (i.e., “I see two eyes, a nose, and a mouth”; Tanaka & Simonyi, 2016; see also Rossion, 2008 for a review). While some authors suggest that this face inversion effect may be due to an innate attentiveness to face-like objects \colorbox{yellow}{(references)}, and that inverted faces are simply too different to recognise as such \colorbox{yellow}{(references)}, others suggest that this processing difference is due to our relative inexperience in viewing inverted faces (references), as holistic processing seems to depend on familiarity with a stimulus class (e.g., Campbell & Tanaka, 2018; Tanaka & Simonyi, 2016; Wong et al., 2009; but see McKone & Robbins, 2007 for a critique). Although the face inversion effect is incredibly robust to task demands and has been widely studied in the context of visual memory (see Tanaka & Simonyi, 2016, and Valentine, 1988 for reviews), no studies have examined the effect of face inversion on gist perception and ensemble coding in an RSVP stream (but see Haberman et al., 2009; Haberman, Lee, & Whitney, 2015; and Leib, Puri, Fischer, Bentin, Whitney, & Robertson, 2012, in relation to ensemble coding generally); and so it is unknown how the perceptual processes that typically allow rapid face recognition in these contexts may operate with inverted faces.
...
- "fingerprint decisions appear to be influenced by similarity to prior cases (Searston, Tangen, & Eva, 2016), suggesting a reliance on memory and a feeling of familiarity when matching unfamiliar images"

One commonly cited factor affecting fingerprint recognition is the ability to deliberately analyse fingerprints (Busey & Parada, 2010; Cole, 2005). Traditionally, fingerprint examiners commence by carefully examining the crime scene 'latent' print and marking up any potentially distinguishing features \colorbox{yellow}{(Emerick et al., 2015)}, such as the overall ridge flow, specific ridge paths and endings, or the ridges' shape and texture (Ashbaugh, 1999). Given that even two fingerprints left by the same finger may vary drastically across contexts (e.g., smudge, distortion; PCAST, 2016), careful analysis of the features in latent prints is typically used to determine if there is enough similarity to the comparison prints in order to make an identification (Emerick et al., 2015). Additionally, the benefit of careful analysis within the laboratory suggests that this is not just an untested forensic procedure - one experiment by Thompson and Tangen \colorbox{yellow}{(Experiment 3; 2014)} found that when examining highly similar, non-matching fingerprints (??) for two seconds compared to 60 seconds, both experts and novices performed better when given more time to examine the fingerprints. This suggests that a major factor influencing identification accuracy may be the opportunity to carefully analyse different fingerprints.

However, it is also plausible that experience in viewing many kinds of fingerprint images can also facilitate recognition. While Experiment 3 of the aforementioned study does suggest a benefit for careful analysis, the results from all experiments combined suggests that fingerprint examiners’ superior identification skills may also derive from non-analytic processes that emerge from vast exposure to many different fingerprints over time (Searston & Tangen, 2017a, 2017b; Thompson & Tangen, 2014). Indeed, the study suggests that \colorbox{yellow}{______.} The authors subsequently suggest that fingerprint training could perhaps be streamlined by exposing novices to varying instances of fingerprints, rather than just carefully comparing individual prints (Thompson & Tangen, 2014). It is possible, therefore, that presenting novices with rapid streams of varying fingerprints will help to simulate experts’ experience, therefore improving their fingerprint recognition ability and developing ‘expertise’ more efficiently.

Specificity?

Describe same person study? Searston & Tangen, 2017


## The Current Study
The present research examines whether viewing an RSVP stream of images at varying rates can better facilitate object recognition compared to viewing a single image, when presented for an equal duration of time. In both studies, participants are presented with complex, unfamiliar images representing the same person (i.e., a stranger's face or fingerprint), which will be presented as single images, or as RSVP streams at varying rates (i.e., two, four, and eight images per second) for a total of eight seconds. In each trial they will be asked whether they viewed images from the same or different category to the test image (e.g., "Is this the same person?"). Based on previous research, we expect that recognition performance will increase as participants view more images per second, given that this would allow them to create richer ensemble representations compared to other conditions. In essence, viewing more images per second may allow participants to become "more familiar" with the unfamiliar stimuli presented, making it easier to to recognise any common features shared with the test image and make an appropriate identification or rejection.

In Experiment 1, participants view the RSVP streams *before* viewing the test image, as previous research suggests \colorbox{yellow}{(references)} that it is the accumulation of multiple previous exemplars that facilitates face recognition. We will also explore whether recognition in these different conditions is affected by familiarity with the general stimulus class, by presenting the faces as upright and inverted images. In doing so, we can examine the limits of ensemble coding: if ensemble coding tends to be automatic and accurate when viewing simple stimuli (e.g., circle size; references) and complex familiar stimuli (e.g., upright faces; references), would it operate the same way when viewing complex *unfamiliar* stimuli (i.e., inverted faces)? We predict that any benefit derived from ensemble coding may actually be more pronounced when viewing inverted compared to upright faces, given that our existing advantage for upright face-matching, and a possible interference from the FFDE with upright faces, may produce a psychological limit as to how beneficial an RSVP stream may be for this stimulus class, which may not be present for inverted faces.

Experiment 2 employs a similar design to Experiment 1; however, to more closely resemble fingerprint identification procedure, participants were shown the target image of a crime scene print first, before viewing the RSVP stream or single comparison print. While this may change the nature of how beneficial the subsequent ensemble representation may be, previous research using the RSVP methodology suggests that when participants are primed to recognise a particular image among a subsequently presented image stream, performance improves drastically \colorbox{yellow}{(reference)}, as they now know what to look for - and so nevertheless, similar to Experiment 1, we predict that performance will improve when viewing more rapid image streams. Additionally, instead of presenting fingerprints in an upright or inverted orientation as in Experiment 1, our conditions manipulated whether participants viewed fingerprints belonging to the same finger (i.e., "Is this John's thumb?"), or to the same person more generally (i.e., "Does this fingerprint belong to John?"). This allows us to examine whether the potential benefits of ensemble coding differentially effects identification decisions made on a specific versus broader level. Given that no studies have directly compared performance in distinguishing prints from same finger versus the same person (but see Searston & Tangen, 2017c, Tangen, Thompson, & McCarthy, 2011, and Thompson, Tangen, & McCarthy, 2014 for indirect comparisons), our study will be the first to do so. While evidence suggests that novices may perform similarly when discriminating prints from the same person and same finger (Searston & Tangen, 2017c; Tangen et al., 2011; Tangen et al., 2014), RSVP streams consisting of the same finger prints may contain less variation compared to prints from different fingers from the same person, and therefore may generate a more stable ensemble with which to compare the latent print (see Whitney & Leib, 2018), making recognition easier. We therefore predict that any benefits derived from the RSVP methodology may be more pronounced when viewing streams of the same finger.
